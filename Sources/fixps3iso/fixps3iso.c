...
static int patch_eboot_sprx_self(u32 lba, char *filename)
{
    
    u16 fw_min = 42100;
    u16 fw_max = 49500;
	u16 patch_fw_version = ((firmware>>12) & 0xF) * 10000 + ((firmware>>8) & 0xF) * 1000 + ((firmware>>4) & 0xF) * 100;
    u32 offset_fw;
    u16 cur_fw_version = 0;
//    int flag = 0;

    u64 file_pos = ((u64) lba * 2048ULL);

    //if(firmware < 0x421C || firmware >= 0x495C) return 0;

    // open EBOOT/SPRX/SELF and changes the fw version
    ...
    if(read_split(file_pos + 0xCULL, (void *) &offset_fw, (int) 4) < 0) {
        printf("\nError!: Reading EBOOT/SPRX/SELF file in ISO\n\n");
        return -1;
    }
    offset_fw = SWAP32(offset_fw);
    offset_fw+= 0x1E;

	// get current firmware version of EBOOT/SPRX/SELF
    if(read_split(file_pos + ((u64) offset_fw), (void *) &cur_fw_version, (int) 2) < 0) {
        printf("\nError!: Reading EBOOT/SPRX/SELF file in ISO\n\n");
        return -1;
    }
    cur_fw_version = SWAP16(cur_fw_version);
               
    //if(firmware >= 0x421C && firmware < 0x495C && cur_fw_version > fw_421 && cur_fw_version <= fw_495 && cur_fw_version > patch_fw_version) {
	// if (cur_fw_version >= 4.21) AND (patch_fw_version between fw_min and fw_max) then PATCH
	if( patch_fw_version >= fw_min && patch_fw_version <= fw_max ) {
        patch_fw_version = SWAP16(patch_fw_version);
        if(write_split2(file_pos + ((u64) offset_fw), (void *) &patch_fw_version, (int) 2) < 0) {
            printf("\nError!: Writing EBOOT/SPRX/SELF file in ISO\n\n");
            return -1;
        }
        printf("Version changed from %u.%u to %u.%u in %s\n\n", cur_fw_version/10000, (cur_fw_version % 10000)/100, patch_fw_version/10000, (patch_fw_version % 10000)/100, filename);
        self_sprx_patched++;
//        flag = 1;
        

//    } else if(cur_fw_version > patch_fw_version) {
//         printf("\nError!: This SELF/SPRX uses a bigger cur_fw_versionsion of %u.%uC (%u.%u)\n\n", patch_fw_version/10000, (patch_fw_version % 10000)/100, cur_fw_version/10000, (cur_fw_version % 10000)/100);
//        flag = -1; // 
    }
...
static int patch_param_sfo(u32 lba, u32 len)
{
    u32 pos, str;
    ...
    sprintf(str_version, "%2.2u.%4.4u", patch_fw_version/10000, patch_fw_version % 10000 );
    ...
    if(param_patched) {
        if(write_split2(file_pos, (void *) mem, (int) len) < 0) {
            printf("\nError!: Writing PARAM.SFO in ISO\n\n");
            return -1;
        }
    }
            ...
            while(1) {
                ...
                if((int) idr->name_len[0] > 1 && idr->flags[0] != 0x2 &&
                    ...
                    if(!strcmp(string, "PARAM.SFO")) {
                        patch_param_sfo(file_lba, file_size);
                        goto next_file;
                    }
                    ...
                    if((string[ ext - 1 ] == '.' && ((string[ ext ] == 's' && string[ ext + 1 ] == 'p' && string[ ext + 2 ] == 'r' && string[ ext + 3 ] == 'x') 
                        ...
                        if(patch_eboot_sprx_self(file_lba, string) < 0) {
